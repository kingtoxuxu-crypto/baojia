...
const historyKey = "toilet-bom-quote-history-v7";
...
function getShouldCostBreakdownRows(totals) {
  return [
    ...
    { label: "Profit 利润及预留", rmb: totals.profitPerUnit, usd: totals.exchangeRate ? totals.profitPerUnit / totals.exchangeRate : 0, note: "内部利润、外购利润及风险预留" }
  ];
}
...
function getFullGovernanceData(totals) {
  ...
  const grossMarginRate = formalPrice ? ((formalPrice - totalCost) / formalPrice) * 100 : 0;
  ...
  const floorPrice = floorGrossMarginRate >= 100
    ? 0
    : (1 - (floorGrossMarginRate / 100)) > 0
      ? totalCost / (1 - (floorGrossMarginRate / 100))
      : 0;
  const approvalPrice = targetGrossMarginRate >= 100
    ? 0
    : (1 - (targetGrossMarginRate / 100)) > 0
      ? totalCost / (1 - (targetGrossMarginRate / 100))
      : 0;
  ...
  const note = status === "需特批"
    ? "当前正式价低于底线毛利，必须填写特批原因并走升级审批。"
    : status === "需审批"
      ? "当前正式价未达到目标毛利，建议走常规审批流程。"
      : "当前正式价满足目标毛利，可按常规流程提交。";
...
function renderFullGovernance(totals) {
  ...
  refs.fullGovernancePanel.innerHTML = `
    ...
    <article class="governance-card">
      ...
      <p class="governance-note">满足目标毛利 ${escapeHtml(formatNumber(governance.targetGrossMarginRate, 2))}% 所需的建议审批价。</p>
    </article>
    <article class="governance-card">
      <span class="governance-label">底价</span>
      <strong class="governance-value">${escapeHtml(formatUsd(governance.floorPrice))}</strong>
      <p class="governance-note">低于该价格将跌破底线毛利 ${escapeHtml(formatNumber(governance.floorGrossMarginRate, 2))}% 。</p>
    </article>
    <article class="governance-card ${toneClass}">
      <span class="governance-label">毛利治理</span>
...
function applySmartDefaults() {
  ...
  state.summary.inlandFreight = smart.inlandFreightPerUnit;
  state.summary.customsFee = smart.customsFeePerUnit;
  state.summary.portCharges = smart.portChargesPerUnit;
  state.summary.inspectionFee = smart.inspectionFreightPerUnit;
  state.summary.oceanFreight = smart.oceanFreightPerUnit;
  state.summary.insuranceFee = smart.insuranceFeePerUnit;
  state.summary.destinationDelivery = smart.destinationDeliveryPerUnit;
...
function getSummaryValues() {
  ...
  const inlandFreightPerUnit = Math.max(Number(state.summary.inlandFreight) || 0, 0);
  const customsFeePerUnit = Math.max(Number(state.summary.customsFee) || 0, 0);
  const portChargesPerUnit = Math.max(Number(state.summary.portCharges) || 0, 0);
  const inspectionFeePerUnit = Math.max(Number(state.summary.inspectionFee) || 0, 0);
  const oceanFreightPerUnit = Math.max(Number(state.summary.oceanFreight) || 0, 0);
  const insuranceFeePerUnit = Math.max(Number(state.summary.insuranceFee) || 0, 0);
  const destinationDeliveryPerUnit = Math.max(Number(state.summary.destinationDelivery) || 0, 0);
  const localChargesPerUnit = inlandFreightPerUnit + customsFeePerUnit + portChargesPerUnit + inspectionFeePerUnit;
  const tradeMeta = getTradeTermMeta();
  const includedExportPerUnit = tradeMeta.includeExport ? localChargesPerUnit : 0;
  const includedOceanPerUnit = tradeMeta.includeOcean ? oceanFreightPerUnit : 0;
  const includedInsurancePerUnit = tradeMeta.includeInsurance ? insuranceFeePerUnit : 0;
  const includedDestinationPerUnit = tradeMeta.includeDestination ? destinationDeliveryPerUnit : 0;
  const quotedLogisticsPerUnit = includedExportPerUnit + includedOceanPerUnit + includedInsurancePerUnit + includedDestinationPerUnit;
  const excludedChargesPerUnit = localChargesPerUnit + oceanFreightPerUnit + insuranceFeePerUnit + destinationDeliveryPerUnit - quotedLogisticsPerUnit;
  ...
  const internalProfitPerUnit = internalBasePerUnit * (internalProfitRate / 100);
  const externalProfitPerUnit = externalBasePerUnit * (externalProfitRate / 100);
  const commissionPerUnit = totalRmbPerUnit * (commissionRate / 100);
  const afterSalesPerUnit = totalRmbPerUnit * (afterSalesRate / 100);
  const fxSafetyPerUnit = totalRmbPerUnit * (fxSafetyRate / 100);
  const profitPerUnit = internalProfitPerUnit + externalProfitPerUnit + commissionPerUnit + afterSalesPerUnit + fxSafetyPerUnit;
  const exchangeRate = Math.max(Number(state.summary.exchangeRate) || 0, 0);
  const usdCostPerUnit = exchangeRate ? totalRmbPerUnit / exchangeRate : 0;
  const usdSellingPerUnit = exchangeRate ? (totalRmbPerUnit + profitPerUnit) / exchangeRate : 0;
...
function getTierQuoteRows(totals) {
  ...
  return tiers.map((quantity) => {
    ...
    const tierTotalRmbPerUnit = tierOperatingCostPerUnit + totals.quotedLogisticsPerUnit;
    const tierInternalBasePerUnit = tierTotalRmbPerUnit - tierExternalBasePerUnit;
    const tierProfitPerUnit = (tierInternalBasePerUnit * (totals.internalProfitRate / 100))
      + (tierExternalBasePerUnit * (totals.externalProfitRate / 100))
      + (tierTotalRmbPerUnit * (totals.commissionRate / 100))
      + (tierTotalRmbPerUnit * (totals.afterSalesRate / 100))
      + (tierTotalRmbPerUnit * (totals.fxSafetyRate / 100));
    const tierUsdSellingPerUnit = totals.exchangeRate ? (tierTotalRmbPerUnit + tierProfitPerUnit) / totals.exchangeRate : 0;
    return {
      quantity,
      tierProductCostPerUnit: tierTotalRmbPerUnit,
      tierUsdSellingPerUnit,
      tierOrderAmount: tierUsdSellingPerUnit * quantity
    };
  });
}
...
function renderSummary(options = {}) {
  ...
  const sections = [
    ...
    },
    {
      key: "summary-profit",
      title: "四、利润与风险",
      desc: "在报价基准成本之上，再叠加利润、佣金、售后和汇率安全垫。",
      summaryPerUnit: formatRmb(totals.profitPerUnit),
      summaryTotal: formatRmb(totals.profitTotal),
      rows: [
        { label: "内部成本基数", desc: "报价基准成本扣除外购基数后，作为内部利润率基础", perUnit: formatRmb(totals.internalBasePerUnit), total: formatRmb(totals.internalBasePerUnit * totals.orderQuantity) },
        { label: "外购成本基数", desc: "外购配件、包装与认证摊销，作为外购利润率基础", perUnit: formatRmb(totals.externalBasePerUnit), total: formatRmb(totals.externalBasePerUnit * totals.orderQuantity) },
        { label: "内部利润", desc: `内部基数 × ${formatNumber(totals.internalProfitRate, 2)}%`, perUnit: formatRmb(totals.internalProfitPerUnit), total: formatRmb(totals.internalProfitPerUnit * totals.orderQuantity) },
        { label: "外购利润", desc: `外购基数 × ${formatNumber(totals.externalProfitRate, 2)}%`, perUnit: formatRmb(totals.externalProfitPerUnit), total: formatRmb(totals.externalProfitPerUnit * totals.orderQuantity) },
        { label: "业务佣金预留", desc: `人民币总成本 × ${formatNumber(totals.commissionRate, 2)}%`, perUnit: formatRmb(totals.commissionPerUnit), total: formatRmb(totals.commissionPerUnit * totals.orderQuantity) },
        { label: "售后风险预留", desc: `人民币总成本 × ${formatNumber(totals.afterSalesRate, 2)}%`, perUnit: formatRmb(totals.afterSalesPerUnit), total: formatRmb(totals.afterSalesPerUnit * totals.orderQuantity) },
        { label: "汇率安全垫预留", desc: `人民币总成本 × ${formatNumber(totals.fxSafetyRate, 2)}%`, perUnit: formatRmb(totals.fxSafetyPerUnit), total: formatRmb(totals.fxSafetyPerUnit * totals.orderQuantity) },
        { label: "利润及预留合计", desc: "内部利润 + 外购利润 + 佣金 + 售后 + 汇率安全垫", perUnit: formatRmb(totals.profitPerUnit), total: formatRmb(totals.profitTotal) }
      ]
    },
    {
      key: "summary-final-quote",
      title: "五、最终报价",
      desc: "把报价基准成本与利润折算成最终外销报价。",
      ...
      rows: [
        ...
        { label: "USD 美金销售价", desc: "（报价基准成本 + 利润及预留）/ 汇率", perUnit: formatUsd(totals.usdSellingPerUnit), total: formatUsd(totals.usdSellingTotal) }
      ]
    },
    {
...
function saveQuoteHistory() {
  ...
  syncSummaryState();
  const totals = getSummaryValues();
  const rapidDecision = getRapidDecisionData(totals);
  const governance = getFullGovernanceData(totals);
  const history = getQuoteHistory();
  const snapshot = {
    ...state,
    savedAt: new Date().toISOString(),
    historyTotal: totals.usdSellingTotal,
    totalsSnapshot: totals,
    rapidDecisionSnapshot: rapidDecision,
    governanceSnapshot: governance
  };
  const nextHistory = [snapshot, ...history.filter((item) => item.quoteCode !== state.quoteCode)].slice(0, 10);
...
function renderHistory() {
  const history = getQuoteHistory();
  if (!history.length) {
    refs.historyList.innerHTML = `<div class="empty-state">暂无历史报价，点击“保存报价”后会显示在这里。</div>`;
    return;
  }

  refs.historyList.innerHTML = "";
  history.forEach((entry) => {
    const node = refs.historyItemTemplate.content.firstElementChild.cloneNode(true);
    const quoteMode = entry.form?.quoteMode === "full" ? "完整模式" : "极速模式";
    const quoteStage = entry.form?.quoteStage === "formal" ? "正式核价" : "预估报价";
    const historyTags = [
      `<span class="history-chip ${entry.form?.quoteMode === "full" ? "mode-full" : "mode-rapid"}">${escapeHtml(quoteMode)}</span>`,
      `<span class="history-chip ${entry.form?.quoteStage === "formal" ? "stage-formal" : "stage-estimate"}">${escapeHtml(quoteStage)}</span>`
    ];
    if (entry.form?.sourceQuoteMode === "rapid" || entry.form?.convertedFromRapidAt) {
      historyTags.push(`<span class="history-chip converted">承接快报</span>`);
    }
    node.dataset.code = entry.quoteCode;
    node.querySelector(".history-code").textContent = entry.quoteCode;
    node.querySelector(".history-meta").textContent = `${entry.form?.customerName || "未命名客户"} ｜ ${entry.form?.productName || "未命名产品"} ｜ ${entry.form?.orderQuantity || 0} 件 ｜ ${new Date(entry.savedAt).toLocaleString("zh-CN")}`;
    node.querySelector(".history-tags").innerHTML = historyTags.join("");
    node.querySelector(".history-total").textContent = formatUsd(entry.historyTotal || 0);
    refs.historyList.appendChild(node);
  });
}

function loadHistoryQuote(code) {
  const entry = getQuoteHistory().find((item) => item.quoteCode === code);
  if (!entry) {
    return;
  }

  state.quoteCode = entry.quoteCode;
  state.form = {
    ...createDefaultForm(),
    ...(entry.form || {})
  };
  state.rows = Array.isArray(entry.rows) && entry.rows.length ? entry.rows : createRowsFromTemplate(state.form.templateSelect);
  state.budgetRows = Array.isArray(entry.budgetRows) && entry.budgetRows.length ? entry.budgetRows : createBudgetRowsFromTemplate(state.form.templateSelect, state.form.productionQuantity);
  state.summary = {
    ...state.summary,
    ...(entry.summary || {})
  };
  state.priceLibrary = mergeDeepPriceLibrary(createDefaultPriceLibrary(), entry.priceLibrary || state.priceLibrary || {});
  state.collapsedCategories = entry.collapsedCategories || {};
...
function buildExportWorkbookHtml(totals) {
  ...
  const basicInfoRows = [
    ...
    ["贸易条款", state.form.tradeTerm || "-", "目标市场", state.form.destinationMarket || "-", "装柜方式", state.form.loadingMode || "-"]
  ];
  ...
  const costBreakdownRows = [
    ...
    ["内部利润", formatRmb(totals.internalProfitPerUnit), formatRmb(totals.internalProfitPerUnit * totals.orderQuantity), `内部基数 x ${formatPercent(totals.internalProfitRate)}`],
    ["外购利润", formatRmb(totals.externalProfitPerUnit), formatRmb(totals.externalProfitPerUnit * totals.orderQuantity), `外购基数 x ${formatPercent(totals.externalProfitRate)}`],
    ["业务佣金预留", formatRmb(totals.commissionPerUnit), formatRmb(totals.commissionPerUnit * totals.orderQuantity), `成本基数 x ${formatPercent(totals.commissionRate)}`],
    ["售后风险预留", formatRmb(totals.afterSalesPerUnit), formatRmb(totals.afterSalesPerUnit * totals.orderQuantity), `成本基数 x ${formatPercent(totals.afterSalesRate)}`],
    ["汇率安全垫", formatRmb(totals.fxSafetyPerUnit), formatRmb(totals.fxSafetyPerUnit * totals.orderQuantity), `成本基数 x ${formatPercent(totals.fxSafetyRate)}`],
    ["利润及预留合计", formatRmb(totals.profitPerUnit), formatRmb(totals.profitTotal), "利润 + 佣金 + 风险预留"],
    ["USD 成本", formatUsd(totals.usdCostPerUnit), formatUsd(totals.usdCostTotal), `按汇率 ${formatNumber(totals.exchangeRate, 4)} 折算`],
    ["USD 报价", formatUsd(totals.usdSellingPerUnit), formatUsd(totals.usdSellingTotal), "客户可见报价口径"]
  ];
...
<head>
  ...
  <style>
    body { font-family: "Plus Jakarta Sans", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background: #f4f6f8; color: #0f172a; margin: 0; padding: 24px; }
    ...
    .hero-title { font-size: 28px; font-weight: 800; margin: 8px 0 6px; font-family: "Cabinet Grotesk", sans-serif; letter-spacing: -0.02em; }
  ...
  <main class="sheet">
    ...
    <section class="section">
      ...
      <table>
        ...
        <tr><td class="label">Trade Term</td><td>${escapeHtml(state.form.tradeTerm || "-")}</td><td class="label">Market</td><td>${escapeHtml(state.form.destinationMarket || "-")}</td></tr>
      </table>
    </section>
    ...
    <section class="section">
      ...
      <div class="summary">
        ...
        <div><span>Load Qty</span><strong>${escapeHtml(totals.estimatedLoadQty ? `${totals.estimatedLoadQty}/40HQ` : "-")}</strong></div>
    ...
    <section class="section">
      ...
      <table>
        ...
        <tbody>
          ${tierRows.map((row) => `<tr><td>${escapeHtml(`${row.quantity} pcs`)}</td><td class="price">${escapeHtml(formatUsd(row.tierUsdSellingPerUnit))}</td><td>${escapeHtml(formatUsd(row.tierOrderAmount))}</td><td>${escapeHtml(state.form.tradeTerm || "-")}</td></tr>`).join("")}
        </tbody>
      </table>
    </section>
...
function bindEvents() {
  ...
  refs.historyList.addEventListener("click", (event) => {
    ...
    if (item) {
      loadHistoryQuote(item.dataset.code);
    }
  });
