(function () {
  ...
  const refs = {
    offlineStatusBadge: document.getElementById("offlineStatusBadge"),
    ...
    paymentTermsInput: document.getElementById("paymentTermsInput"),
    quoteNoteInput: document.getElementById("quoteNoteInput"),
  ...
  async function saveSettings(settings) {
    ...
    renderAccountInfo();
    setInlineMessage(refs.settingsMessage, "默认设置已保存，将用于客户版 PDF/打印。", false);
  ...
  function getQuoteActionButtons(row) {
    ...
    if (row.status === "approved") {
      actions.push(`<button class="secondary quote-action-btn quote-action-secondary" type="button" data-action="mark-quote-sent" data-id="${escapeHtml(row.id)}">标记已发送</button>`);
    }
    if (row.status === "sent") {
      actions.push(`<button class="secondary quote-action-btn quote-action-secondary" type="button" data-action="mark-quote-final" data-id="${escapeHtml(row.id)}">设为最终确认</button>`);
    }
  ...
  function buildDashboardRecentRows() {
    ...
    return indexedRows.sort((a, b) => b.sortTime - a.sortTime).slice(0, 6);
  }
  ...
  function renderDashboard() {
    ...
    refs.dashboardUserCount.textContent = formatCount(4);
  ...
  function renderAccountInfo() {
    ...
    refs.companyNameInput.value = settings.companyName || "";
    refs.paymentTermsInput.value = settings.paymentTerms || "";
    refs.quoteNoteInput.value = settings.quoteNote || "";
  }
  ...
  function isFrozenQuoteStatus(status) {
    return ["approved", "final"].includes(String(status || "").toLowerCase());
  }
  ...
  function bindEvents() {
    refs.importBackupBtn.addEventListener("click", importBackup);
    refs.exportBackupBtn.addEventListener("click", () => exportBackup().catch((error) => setCloudStatus(`导出失败：${escapeHtml(error.message)}`, "error")));
    ...
    refs.cloudQuoteList.addEventListener("click", async (event) => {
      ...
      try {
        if (action === "open-cloud-quote") {
          ...
          await updateQuoteStatus(quoteId, "pending");
          ...
          await updateQuoteStatus(quoteId, "final");
    ...
    refs.settingsForm.addEventListener("submit", async (event) => {
      event.preventDefault();
      await saveSettings({
        companyName: refs.companyNameInput.value.trim(),
        paymentTerms: refs.paymentTermsInput.value.trim(),
        quoteNote: refs.quoteNoteInput.value.trim()
      });
    });
  }
  ...
  async function initOffline() {
    refs.actionModal?.classList.add("hidden");
    refs.offlineStatusBadge.textContent = "离线模式 / 本机保存";
    refs.importBackupBtn.textContent = "导入备份";
    refs.exportBackupBtn.textContent = "导出备份";
    refs.exportBackupBtn.hidden = false;
    refs.cloudSaveBtn.disabled = false;
  ...
  window.offlineQuoteStore = {
    saveCurrentQuoteOffline,
    refreshOfflineQuotes,
    exportBackup,
    importBackup,
    getQuoteSnapshots: () => offlineQuotes.map((row) => ({
      quoteCode: row.quote_code,
      form: row.form_json || {},
      totals: row.totals_json || {},
      totalsSnapshot: row.totals_json || {},
